...
-- Unit leadership roles: they administer their own unit, never the foundation or another unit.
...
create or replace function public.is_unit_manager() returns boolean language sql stable security definer set search_path=public
as $$ select exists (select 1 from public.profiles where id=(select auth.uid()) and role in ('admin_unit'::public.user_role,'kepala_sekolah'::public.user_role,'wakil_kepala_sekolah'::public.user_role)) $$;
...
create or replace function public.can_manage_unit(target_unit uuid) returns boolean language sql stable security definer set search_path=public
...
create or replace function public.can_manage_class(target_class uuid) returns boolean language sql stable security definer set search_path=public
as $$ select public.is_admin_yayasan() or exists (select 1 from public.classes c where c.id=target_class and public.is_unit_manager() and c.unit_id=public.current_user_unit()) $$;
...
create or replace function public.can_manage_student(target_student uuid) returns boolean language sql stable security definer set search_path=public
as $$ select public.is_admin_yayasan() or exists (select 1 from public.students s where s.id=target_student and public.is_unit_manager() and s.current_unit_id=public.current_user_unit()) $$;
