...
export default function YayasanAdmin() {
  const { isAdminYayasan, profile } = useAuth()
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: u }, { data: y }, { data: se }, { data: c }, { data: su }, { data: p }, { count: profiles }, { count: students }] = await Promise.all([
      ...
      supabase.from('profiles').select('id, full_name, role, unit_id').in('role', ['guru', 'wali_kelas', 'admin_unit', 'kepala_sekolah', 'wakil_kepala_sekolah']).order('full_name'),
  ...
  const managedUnits = useMemo(() => isAdminYayasan ? units : units.filter((u) => u.id === profile?.unit_id), [isAdminYayasan, units, profile?.unit_id])
  const managedClasses = useMemo(() => isAdminYayasan ? classes : classes.filter((c) => c.unit_id === profile?.unit_id), [isAdminYayasan, classes, profile?.unit_id])
  const managedSubjects = useMemo(() => isAdminYayasan ? subjects : subjects.filter((s) => s.unit_id === profile?.unit_id), [isAdminYayasan, subjects, profile?.unit_id])
  const managedTeachers = useMemo(() => isAdminYayasan ? teachers : teachers.filter((t) => t.unit_id === profile?.unit_id), [isAdminYayasan, teachers, profile?.unit_id])
  if (loading) return <FullPageSpinner />

  return <div className="space-y-6">
    <PageHeader title={isAdminYayasan ? 'Admin Yayasan' : 'Pengaturan Sekolah'} description={isAdminYayasan ? 'Kelola struktur sekolah dan data awal project Supabase GPT.' : 'Kelola data akademik untuk unit Anda. Data unit, akun, dan peran dikelola Admin Yayasan.'} />
