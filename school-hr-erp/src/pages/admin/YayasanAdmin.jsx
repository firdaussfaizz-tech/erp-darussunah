...
export default function YayasanAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: u }, { data: y }, { data: se }, { data: c }, { data: su }, { data: p }, { count: profiles }, { count: students }] = await Promise.all([
      ...
      supabase.from('classes').select('id, unit_id, academic_year_id, name, homeroom_teacher_id, units(code, name), academic_years(name), homeroom:homeroom_teacher_id(full_name)').order('name'),
