...
export default function Layout() {
  ...
  return <div className="app-shell">
    ...
    <header className="global-header"><div className="global-nav">
      ...
      <nav className="desktop-nav" aria-label="Navigasi utama">{navigation.map(([path,label]) => <NavLink key={path} to={path} end={path === '/'}>{label}</NavLink>)}{isAdminYayasan && <><NavLink to="/admin-yayasan">Admin Yayasan</NavLink><NavLink to="/pengguna">Kelola Hak Akses</NavLink></>}</nav>
    ...
    </div><nav id="mobile-navigation" className={`mobile-nav ${open ? 'is-open' : ''}`} aria-label="Navigasi seluler">{navigation.map(([path,label]) => <NavLink key={path} to={path} end={path === '/'} onClick={() => setOpen(false)}>{label}</NavLink>)}{isAdminYayasan && <><NavLink to="/admin-yayasan" onClick={() => setOpen(false)}>Admin Yayasan</NavLink><NavLink to="/pengguna" onClick={() => setOpen(false)}>Kelola Hak Akses</NavLink></>}<button onClick={logout}>Keluar dari akun</button></nav></header>
