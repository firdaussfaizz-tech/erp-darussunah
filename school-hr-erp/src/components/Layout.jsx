...
export default function Layout() {
  ...
  return <div className="app-shell">
    ...
    </div><nav id="mobile-navigation" className={`mobile-nav ${open ? 'is-open' : ''}`} aria-label="Navigasi seluler">{navigation.map(([path,label]) => label === 'Kesiswaan' ? <div key={path}><NavLink to="/siswa" end onClick={() => setOpen(false)}>Kesiswaan</NavLink><NavLink to="/siswa/profil" onClick={() => setOpen(false)}>↳ Profil siswa & wali</NavLink><NavLink to="/siswa/penempatan" onClick={() => setOpen(false)}>↳ Penempatan kelas</NavLink><NavLink to="/siswa/presensi-izin" onClick={() => setOpen(false)}>↳ Presensi & izin</NavLink></div> : <NavLink key={path} to={path} end={path === '/'} onClick={() => setOpen(false)}>{label}</NavLink>)}{isManager && <NavLink to="/admin-yayasan" onClick={() => setOpen(false)}>{isAdminYayasan ? 'Admin Yayasan' : 'Pengaturan Sekolah'}</NavLink>}{isAdminYayasan && <NavLink to="/pengguna" onClick={() => setOpen(false)}>Kelola Hak Akses</NavLink>}<button onClick={logout}>Keluar dari akun</button></nav></header>
